...
describe('mapStock / mapMenu', () => {
  ...
  it('menu: isActive→active, imageUrl→image, variant {stockId,qty}, addons', () => {
    const m = mapMenu({ id: 1, name: 'A', description: null, basePrice: 10000, isActive: false, isAddon: false, imageUrl: '/uploads/a.png', variants: [{ name: 'Reg', price: 10000, stockId: 2, qty: 3 }], addons: [4] })
    expect(m).toMatchObject({ active: false, image: '/uploads/a.png', addons: [4] })
...
describe('mapOrderRow / mapOrderDetail', () => {
  ...
  it('detail: item snapshot → {name,meta,addon}, preOrder → poTitle/poDate', () => {
    const d = mapOrderDetail({
      ...
      preOrder: { title: 'PO Open', fulfillmentDate: '2026-06-14T05:00:00Z' },
    })
    ...
    expect(d).toMatchObject({ poTitle: 'PO Open', poDate: '14 Jun 2026', adminNotes: '' })
  })
})

describe('mapPreorderRow / mapCustomerRow / mapSubscriber / mapSetting / mapUser', () => {
  it('preorder row: date + stats', () => {
    expect(mapPreorderRow({ id: 2, title: 'PO', description: null, status: 'open', fulfillmentDate: '2026-06-14T05:00:00Z', fulfillmentNote: null, orderCount: 3, revenue: 90000 }))
      .toMatchObject({ status: 'open', date: '14 Jun 2026', note: '—', orderCount: 3, revenue: 90000 })
  })
...
describe('mapDashboard', () => {
  it('kpis + openPreorder date + recent + lowStock', () => {
    const d = mapDashboard({
      ...
      openPreorder: { id: 1, title: 'PO Open', fulfillmentDate: '2026-06-14T05:00:00Z', fulfillmentNote: 'n' },
    ...
    expect(d.openPreorder).toMatchObject({ title: 'PO Open', date: '14 Jun 2026', note: 'n' })
...
describe('menuToApi', () => {
  const draft: MenuDraft = { name: 'X', description: 'desc', basePrice: '12000', active: false, isAddon: false, image: '/uploads/x.png', variants: [{ name: 'Reg', price: 12000, stockId: 1, qty: 2 }], addons: [3, 4] }
  it('active→isActive, image→imageUrl, basePrice parse', () => {
    expect(menuToApi(draft)).toMatchObject({ name: 'X', basePrice: 12000, isActive: false, imageUrl: '/uploads/x.png', variants: [{ name: 'Reg', price: 12000, stockId: 1, qty: 2 }], addons: [3, 4] })
  })
  it('menu add-on → addons dikosongkan', () => {
    expect(menuToApi({ ...draft, isAddon: true }).addons).toEqual([])
  })
})
